      ...
      toastBC.current.show({
        ...
        content: (props) => (
          ...
          >
            ...
            <div className="font-medium text-lg my-3 text-800">
              {props.message.summary}
            </div>
            <div className="flex justify-between w-full">
              <Link href="/pages/forecast" legacyBehavior passHref>
                <Button
                  className="p-button-sm w-full ml-2"
                  label={button}
                  severity="success"
  ...
  const existFolder = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        if (success) {
          const summaryText = exists
            ? "¿Desea acceder a la carpeta?"
